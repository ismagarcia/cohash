            ...
            libhu::U32             &max_age)
{
  ...
  for (unsigned int block = 0; block < num_blocks; block += max2DBlocks)
  {
...
#if (!DISABLE_NVPA_KERNEL_EVEN_MEMORY_DECLARATION)
    NVPA_START_EVENT_GPU(openclkernelBuildHash);
    ...
    thrust::detail::device::opencl::synchronize_if_enabled("hash_kernel");

#if (!DISABLE_NVPA_KERNEL_EVEN_MEMORY_DECLARATION)
     NVPA_STOP_EVENT_GPU(openclkernelBuildHash);
#endif
                   ...
                   libhu::U32             &max_age)
{
  ...
  thrust::detail::device::opencl::synchronize_if_enabled("hash_by_key_kernel");
