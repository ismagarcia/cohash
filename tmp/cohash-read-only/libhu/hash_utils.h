...
#elif (_M_X64 || __x86_64__  || __arm__)
#include <inttypes.h>
...
#define TBID                get_group_id(0)
#define TBSZ                get_local_size(0)
#define GTID                (get_local_id(0) + get_group_id(0) * get_local_size(0) + \
                            get_group_id(1) * get_local_size(0) * get_num_groups(0))
#define NUM_INPUT_KEYS      (get_local_size(0) * get_local_size(1) * get_num_groups(0))
#define NUM_SET_OF_KEYS     TBSZ
#define SET_OF_KEYS_ID      TBID
...
#else
    old = __sync_val_compare_and_swap((volatile libhu::S64 *)address, assumed, std::max(val,assumed));
#endif
    }
    while (assumed != old);
  }
...
#else
    old = __sync_val_compare_and_swap((volatile libhu::S32 *)address, assumed, std::max(val,assumed));
#endif
    }
    while (assumed != old);
  }
