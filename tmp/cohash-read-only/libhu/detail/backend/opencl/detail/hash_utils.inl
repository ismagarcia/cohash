...
#if (ENABLE_NVPA)
#if (!DISABLE_NVPA_KERNEL_EVEN_MEMORY_DECLARATION)
  NVPA_KERNEL_START_EVENT(openclinternalAccessHash);
#endif
#endif
...
#if (ENABLE_NVPA)
#if (!DISABLE_NVPA_KERNEL_EVEN_MEMORY_DECLARATION)
  NVPA_KERNEL_STOP_EVENT(openclinternalAccessHash);
#endif
#endif
  ...
  void access(T_RAND_ACCESS_ITERATOR1 search_keys_begin,
              ...
              libhu::U32              max_age)
            {
              ...
              for (unsigned int block = 0; block < num_blocks; block += max2DBlocks)
              {
...
#if (ENABLE_NVPA)
#if (!DISABLE_NVPA_KERNEL_EVEN_MEMORY_DECLARATION)
                NVPA_START_EVENT_GPU(openclkernelAccessHash);
                ...
                thrust::detail::device::opencl::synchronize_if_enabled("access_kernel");
#if (ENABLE_NVPA)
#if (!DISABLE_NVPA_KERNEL_EVEN_MEMORY_DECLARATION)
                NVPA_STOP_EVENT_GPU(openclkernelAccessHash);
#endif
#endif
