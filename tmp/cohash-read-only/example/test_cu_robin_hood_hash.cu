...
struct copy_tex_value
{
  ...
  inline __host__ __device__ libhu::U32 GET_KEY_POS(libhu::U64 k) { return ((k) & KEY_TYPE_MASK); }
  inline __host__ __device__ libhu::U32 GET_KEY_ATTACH_ID(libhu::U64 k) { return ((k) >> (KEY_TYPE_BITS)) & ATTACH_ID_TYPE_MASK; }
  ...
  void operator()(T& t)
  {
    ...
    if (t != PACKED_UNDEFINED_KEY)
    {
      ...
      valuesPtr[GET_KEY_POS((W * H) - t)] = refValuesPtr[GET_KEY_POS((W * H) - t)];
    }
  }
...
ANNOTATE_SETUP;
  ...
  {
    char param[128];
    sprintf(param,"%u", getpid());
    {
      pid_t pid = fork();
      if (pid == 0)
      {
        // child process
        execl("/data/local/tmp/cohash/example/trace-cmd", "trace-cmd", "start", "-b", "20000", "-e", "all", "-P", param, "-c", (char *)NULL);
  ...
  std::cerr << "mode                             : " << ((ENABLE_KEY_DATA_MODE == 1) ? "key_value (64b)" : "key (32b)") << " " << ((ENABLE_DEVICE_OMP_COMPUTING == 1) ? "[CPU OpenMP]" : "[CPU single thread]")  <<std::endl;
  ...
  typedef THRUST_VECTOR<T_KEY>::iterator T_KEY_IT;
  typedef THRUST_VECTOR<T_VALUE>::iterator T_VALUE_IT;

#if (!ENABLE_KEY_DATA_MODE)
  typedef libhu::U32 T_HASH_TABLE;
  typedef THRUST_VECTOR<T_HASH_TABLE>::iterator T_HASH_TABLE_IT;
  ...
  THRUST_VECTOR<T_KEY>        values(num_extra);
  THRUST_VECTOR<T_HASH_TABLE> hash_table(hash_table_size);
  ...
  libhu::U32        key_max_value = krf.KEY_TYPE_RANGE;
  libhu::U32          key_max_age = krf.KEY_TYPE_MAX_AGE;
  THRUST_VECTOR<T_VALUE>      values(num_extra);
  THRUST_VECTOR<T_HASH_TABLE> hash_table(hash_table_size);
#endif

  THRUST_VECTOR<T_KEY> keys(num_extra);

  //libhu::print("[pre]  keys", keys.begin(), keys.end());
#if (!ENABLE_KEY_DATA_MODE)
  thrust::fill(keys.begin(), keys.end(), key_coh_hash_functor::UNDEFINED_KEY);
#elif (ENABLE_KEY_DATA_MODE)
  thrust::fill(keys.begin(), keys.end(), key_value_coh_hash_functor::UNDEFINED_KEY);
#endif
  //libhu::print("[post] keys", keys.begin(), keys.end());
  ...
  if (cfg.coh_hash)
  {
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "coh_hash_build_pre");
    #endif

    #if (ENABLE_TRACE_CMD)
      {
        std::string buffer;
        std::ifstream cci_pmu_logger_control("/sys/kernel/debug/cci_pmu_logger/control");
        std::getline(cci_pmu_logger_control, buffer);
        std::cerr << "trace-cmd control state          : " << buffer << std::endl;
        cci_pmu_logger_control.close();
      }
      {
        pid_t pid = fork();
        if (pid == 0) 
        { 
          // child process
          //execl("/system/bin/sh", "sh", "-c", "/data/local/tmp/cohash/example/busybox echo 1 > /sys/kernel/debug/cci_pmu_logger/control", (char *)NULL);
          execl("/bin/sh", "sh", "-c", "echo 1 > /sys/kernel/debug/cci_pmu_logger/control", (char *)NULL);
          exit(127); // only if execv fails
        }
        else 
        { 
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "coh_hash_build_pos");
    #endif
...
#endif
      coh_build_time = ( ( tm_end.tv_sec - tm_start.tv_sec ) + ( tm_end.tv_nsec - tm_start.tv_nsec ) / TIME_1G_BILLION ) * TIME_1K_MILLISECONDS;
     ...
     
    {
...
#if (ENABLE_PERF)
			pmf_perf_counter *counter_c0;
			pmf_perf_counter *counter_c1;
			pid_t pid = getpid();
			int cpu = 0;
			counter_c0 =  new pmf_perf_counter(pmf_perf_counter::L1_MISSES);
			counter_c1 =  new pmf_perf_counter(pmf_perf_counter::L1_MISSES);
	    counter_c0->start();
	    counter_c1->start();
#endif
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "coh_hash_access_pre");
    #endif
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "coh_hash_access_pos");
    #endif
      ...
      cudaEventElapsedTime(&coh_access_time, start, end);
			...
			counter_c0->stop();
			counter_c1->stop();
      std::cerr << "L1 cache misses                  : " << counter_c0->value()+counter_c1->value() << std::endl;
#endif

      coh_access_time = ( ( tm_end.tv_sec - tm_start.tv_sec ) + ( tm_end.tv_nsec - tm_start.tv_nsec ) / TIME_1G_BILLION ) * TIME_1K_MILLISECONDS;

#if (ENABLE_HOST_COMPUTING || ENABLE_DEVICE_OMP_COMPUTING)
  #if (ENABLE_LIBHU_LOG_DETAILED)
      libhu::print("search_keys", keys.begin(), keys.end());
      std::cerr << "hashed_keys: "; libhu::print_keys(hash_table.begin(), hash_table.end(), kcf);
      std::cerr << "hashed_ages: "; libhu::print_ages(hash_table.begin(), hash_table.end(), kcf);
  #endif
#endif
      ...
      if (cfg.image_mode)
      {
        ...
        THRUST_VECTOR<libhu::U32> dev_values(cfg.tex->w * cfg.tex->h);
        THRUST_VECTOR<libhu::U32> dev_refValues(cfg.tex->w * cfg.tex->h);
  ...
  else if (cfg.rand_hash)
  {
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "rand_hash_build_pre");
    #endif
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "rand_hash_build_pos");
    #endif
...
#endif
      rand_build_time = ( ( tm_end.tv_sec - tm_start.tv_sec ) + ( tm_end.tv_nsec - tm_start.tv_nsec ) / TIME_1G_BILLION ) * TIME_1K_MILLISECONDS;
...

    {
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "rand_hash_access_pre");
    #endif
    ...
    #if (ENABLE_GATOR)
      ANNOTATE_MARKER_COLOR_STR(ANNOTATE_BLUE, "rand_hash_access_pos");
    #endif
      ...
      rand_access_time = ( ( tm_end.tv_sec - tm_start.tv_sec ) + ( tm_end.tv_nsec - tm_start.tv_nsec ) / TIME_1G_BILLION ) * TIME_1K_MILLISECONDS;

#if (ENABLE_HOST_COMPUTING || ENABLE_DEVICE_OMP_COMPUTING)
  #if (ENABLE_LIBHU_LOG_DETAILED)
      libhu::print("search_keys", keys.begin(), keys.end());
      std::cerr << "hashed_keys: "; libhu::print_keys(hash_table.begin(), hash_table.end(), krf);
      std::cerr << "hashed_ages: "; libhu::print_ages(hash_table.begin(), hash_table.end(), krf);
  #endif
#endif
  ...
  /*
  {
    ...
    sprintf(param,"%u", getpid());
    {
      ...
      {
        // child process
        execl("/data/local/tmp/cohash/example/trace-cmd", "trace-cmd", "stop", (char *)NULL);
        exit(127); // only if execv fails
      }
      else
      {
