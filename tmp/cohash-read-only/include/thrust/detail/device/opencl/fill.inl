...
// openclRT, without system support
...
template<typename OutputIterator, typename Size, typename T>
  OutputIterator fill_n(OutputIterator first,
                        ...
                        thrust::detail::true_type)
{
  // implement this with openclMemset if value == 0
  ...
  if(std::strncmp(reinterpret_cast<const char*>(&value), reinterpret_cast<const char*>(&zero), sizeof(T)) == 0)
  {
    ...
    openclError_t error = openclMemset(thrust::raw_pointer_cast(&*first), 0, sizeof(OutputType) * n);
    if(error)
    {
                        ...
                        const T &value)
{
  ...
  // to possible implement this with openclMemset, OutputIterator needs to be trivial,
