...
#include <thrust/detail/device/opencl/block/reduce.h>
#include <thrust/detail/device/opencl/extern_shared_ptr.h>
#include <thrust/detail/device/opencl/dispatch/reduce.h>
 ...
 * the elements.
 * 
 * Uses the same pattern as reduce6() in the opencl SDK
 *
 */
                  ...
                  SharedArray shared_array)
{
    ...
    thrust::detail::device::opencl::block::reduce_n(shared_array, min(n - blockDim.x * blockIdx.x, blockDim.x), binary_op);
...
template<typename InputIterator,
  ...
  reduce_n_smem(InputIterator input,
                ...
                BinaryFunction binary_op)
{
    thrust::detail::device::opencl::extern_shared_ptr<OutputType> shared_ptr;
                                  ...
                                  thrust::detail::true_type)   // reduce in shared memory
{
  ...
  const size_t block_size = thrust::detail::device::opencl::arch::max_blocksize_with_highest_occupancy(reduce_n_smem<RandomAccessIterator1, OutputType, BinaryFunction>, smem_per_thread);
                                  ...
                                  thrust::detail::false_type)   // reduce in global memory
{
  ...
  const size_t block_size = thrust::detail::device::opencl::arch::max_blocksize_with_highest_occupancy(reduce_n_gmem<RandomAccessIterator1, OutputType, BinaryFunction>, 0);

  // allocate storage for shared array
  thrust::detail::raw_opencl_device_buffer<OutputType> shared_array(block_size * num_blocks);
                                                        ...
                                                        BinaryFunction binary_op)
{
  ...
  const size_t num_blocks_from_wide_part =
    thrust::detail::device::opencl::get_unordered_blocked_standard_reduce_n_schedule(xfrm_wide_first, n_wide, init, binary_op);
                                                            ...
                                                            BinaryFunction binary_op)
{
  ...
  {
    block_size = thrust::detail::device::opencl::arch::max_blocksize_with_highest_occupancy(detail::reduce_n_smem<RandomAccessIterator, OutputType, BinaryFunction>, smem_per_thread);
  }
  else
  {
    block_size = thrust::detail::device::opencl::arch::max_blocksize_with_highest_occupancy(detail::reduce_n_gmem<RandomAccessIterator, OutputType, BinaryFunction>, smem_per_thread);
  }
  ...
  if(smem_per_thread > 0)
  {
    max_blocks = thrust::detail::device::opencl::arch::max_active_blocks(detail::reduce_n_smem<RandomAccessIterator, OutputType, BinaryFunction>, block_size, smem_size);
  }
  else
  {
    max_blocks = thrust::detail::device::opencl::arch::max_active_blocks(detail::reduce_n_gmem<RandomAccessIterator, OutputType, BinaryFunction>, block_size, smem_size);
  }
                                       ...
                                       RandomAccessIterator2 result)
{
  ...
  if(num_blocks < 2)
  {
    thrust::detail::device::opencl::unordered_blocked_standard_reduce_n(first, n, num_blocks, binary_op, result);
  } // end if
  else
  {
    ...
    thrust::detail::device::opencl::unordered_blocked_standard_reduce_n(xfrm_wide_first, n_wide, num_wide_results, binary_op, result);

    // process tail
    thrust::detail::device::opencl::unordered_blocked_standard_reduce_n(tail_first, n_tail, (size_t)1u, binary_op, result + num_wide_results);
  } // end else
                                  ...
                                  RandomAccessIterator2 result)
{
  ...
  return thrust::detail::device::opencl::dispatch::unordered_blocked_reduce_n(first, n, num_blocks, binary_op, result,
                                                   ...
                                                   BinaryFunction binary_op)
{
  ...
  return thrust::detail::device::opencl::dispatch::get_unordered_blocked_reduce_n_schedule(first, n, init, binary_op,
